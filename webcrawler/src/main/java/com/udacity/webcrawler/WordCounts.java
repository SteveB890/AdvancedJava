...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    ...
    // Create a stream from the wordCounts map, sort the elements of the stream using WordCountComparator().compare,
    // limit the size of the output to popularWordCount, finally collect the stream into a LinkedHashMap
    return wordCounts.entrySet().stream().sorted((p1, p2) -> (new WordCountComparator().compare(p1, p2)))
